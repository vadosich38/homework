...
class Roommates:
    """
    Базовый класс, описывающий сущность жильца дома.

    Глобальные счетчики:
    __income_sum - доход за все время игры.
    __fur_count - количество купленных шуб за все время игры.
    __food_counter - количество съеденной еды за все время игры.
    __cat_food_counter - количество съеденной кошачей еды за все время игры.
    
    Args:
        name(str): передается имя игрового персонажа.
        happy(int): передается уровень счастья персонажа. По умолчанию 100, у персонажей, которым не свойственно счастье
        по умолчанию 0.
    """
    __income_sum = 0
    __fur_count = 0
...
class House:
    """
    Базовый класс, описывающий сущность жилья.

    Args:
        __money(int): количество денег в доме
        __food(int): количество еды в доме
        __cat_food(int): количество кошачей еды дома
        __dirt(int): степень загрязнения дома
    """
    def __init__(self):
        self.__money = 100
        self.__food = 50
...
class Cat(Roommates):
    """
    Класс Кот. Родитель: Roommates

    Args:
        happy(int): уровень счастья не свойствен коту, по умолчанию равен 0
    """
...
class Kid(Roommates):
    """
    Класс Ребенок. Родитель: Roommates

    Args:
        happy(int): уровень счастья ребенка. По умолчанию 100.
